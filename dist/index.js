...
const convertDoublesToInts = metric => {
  ...
  metric.value[1] = _lodash2.default.round(metric.value[1]);
...
const appdRequest = async data => {
  ...
  const response = await (0, _nodeFetch2.default)(APPD_URL, {
    method: 'POST',
    headers: {
...
const prometheusRequest = async query => {
  ...
  const response = await (0, _nodeFetch2.default)(`${PROETHEUS_URL}/api/v1/query?query=${query}`, {
    method: 'GET',
    headers: {
