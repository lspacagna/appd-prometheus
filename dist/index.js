...
const prometheusRequest = async query => {
  ...
  const response = await (0, _nodeFetch2.default)(`${PROMETHEUS_URL}/api/v1/query?query=${query}`, {
    method: 'GET',
    headers: {
...
const getDataFromPrometheus = async () => {
  ...
  const queries = _fs2.default.readFileSync('conf/queries.txt', 'utf8').toString().trim().split("\n");
  ...
  data = _lodash2.default.flatten(data);

  return data;
};

const processConfig = () => {
  const config = JSON.parse(_fs2.default.readFileSync('conf/config.json', 'utf8'));
...
const main = async () => {
  try {
    ...
    if (READ_LOCAL) {
      ...
      data = _fs2.default.readFileSync(LOCAL_FILE, 'utf8');
    ...
    await _analytics2.default.publish({
